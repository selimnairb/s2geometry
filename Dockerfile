...
RUN apt-get update
RUN apt-get install -y git libssl-dev build-essential cmake swig python3-dev
...
RUN cd /tmp && \
    ...
    mkdir build && \
    cmake -S /tmp/abseil-cpp -B /tmp/abseil-cpp/build -DCMAKE_POSITION_INDEPENDENT_CODE=ON -DCMAKE_PREFIX_PATH=/usr -DCMAKE_INSTALL_PREFIX=/usr -DABSL_ENABLE_INSTALL=ON -DABSL_USE_EXTERNAL_GOOGLETEST=ON -DABSL_FIND_GOOGLETEST=ON && \
